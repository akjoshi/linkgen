...
//this is copied from YCSB Generator. com.yahoo.ycsb.generator;
public class ZipfianGenerator {
	 ...
	 * the number of items in the set to draw from, either by specifying an
	 * itemcount (so that the sequence is of items from 0 to itemcount-1) or by
	 * specifying a min and a max (so that the sequence is of items from min to
	 * max inclusive). After you construct the instance, you can change the
	 * number of items by calling nextInt(itemcount) or nextLong(itemcount).
	 * 
	 * Note that the popular items will be clustered together, e.g. item 0 is
	 * the most popular, item 1 the second most popular, and so on (or min is
	 * the most popular, min+1 the next most popular, etc.) If you don't want
	 * this clustering, and instead want the popular items scattered throughout
	 * the item space, then use ScrambledZipfGenerator instead.
	 * 
	 * Be aware: initializing this generator may take a long time if there are
	 * lots of items to choose from (e.g. over a minute for 100 million
	 * objects). This is because certain mathematical values need to be computed
	 * to properly generate a zipfian skew, and one of those values (zeta) is a
	 * sum sequence from 1 to n, where n is the itemcount. Note that if you
	 * increase the number of items in the set, we can compute a new zeta
	 * incrementally, so it should be fast unless you have added millions of
	 * items. However, if you decrease the number of items, we recompute zeta
...
//		System.out.println("countforzeta=" + sum);
		...
		if (itemcount != countforzeta) {
			...
			synchronized (this) {
				if (itemcount > countforzeta) {
					...
					System.err
							.println("WARNING: Recomputing Zipfian distribtion. This is slow and should be avoided. (itemcount="
	 ...
	 * throughout the item space, use ScrambledZipfGenerator instead.
	 */
	public Long nextValue() {
		Long l = nextLong(items);		
		return l;
	}
	...
	public static void main(String[] args) {
		...
		ZipfianGenerator z = new ZipfianGenerator(max_subjects);// max. number of subject
		long count = 0;
		long val=0;
		while(true){
			val=z.nextValue();
			...
			if (count>max_triples) break;
		}
		//z.nextValue();
	}

	/**
	 * @todo Implement ZipfGenerator.mean()
	 */
	public double mean() {
		throw new UnsupportedOperationException("@todo implement ZipfGenerator.mean()");
	}
}
